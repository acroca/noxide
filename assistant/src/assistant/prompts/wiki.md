...
- Tasks are owned by project/area pages (or their sub-pages), under `## Tasks` — never by the journal or people pages. `now.md` additionally mirrors every open task (see its **Tasks** section below); the owning page's copy is the authority:
  ...
  - Wanted but no rush ("some day", "cuando pueda", a date that keeps slipping and never mattered): append `(someday)` **instead of** a date — never both. A someday task never appears under **Today** and is never escalated as overdue or stale; it stays in **Tasks** like any open task and, grouped, under **Someday**, which the lint reads out once a week. Giving it a date later removes the marker; a user saying "no rush" about a dated task removes the date.
...
Fully materialized dashboard — no queries, just text. Sections: **Today** (events, due/overdue routines, tasks due today — and every overdue task, kept listed with its original `(due YYYY-MM-DD)` until done, rescheduled or explicitly dropped: a task vanishing from the dashboard the day after its deadline passes is how a decision silently expires), **Upcoming** (every dated event on a live page within the next 14 days, plus reminders, dated tasks and routine due dates), **Waiting** (blocked on someone/something external), **Tasks** (every open task in the wiki — see below), **Someday** (every `(someday)` task, one line each ending in a link to its owning page — the wanted-but-not-now list, visible without cluttering **Today**; `check_vault` pairs it with the pages both ways), **Last 7 days** (one bullet per day, newest first, synthesizing that day's journal).
...
**Upcoming** mirrors every dated event bullet on a live page whose date falls within the next 14 days — a school calendar's next date, a trip, an appointment on an area page — with the event text copied verbatim, alongside reminders, dated tasks and routine due dates. `check_vault` pairs each page event with a `now.md` line by date and text, so an event the rebuild skipped is a finding the same night. This is also the one place the vault's ISO rule relaxes, because this file is always read whole and the nightly rebuild replaces it entirely: an item **within the next 7 days** is labelled by weekday (`Tuesday — …`, `Friday (evening) → Saturday (early hours) — …`), read against the `Today` header just above it as "the next Tuesday", with the ISO date kept in parentheses at the end (`Tuesday — First day of school (2026-09-08)`) so the line still pairs with its page. Anything **further out keeps its ISO date** up front: a bare weekday three weeks away names four candidate days. The rest of the file stays ISO — the `Today` header (weekday *and* date, the anchor that makes the bare weekdays below it readable), **Waiting** deadlines, and the **Last 7 days** bullets, which are the searchable index of recent days.
...
2. **Update the page that owns the fact.** The project/area page; or `routines.md` (**Last done** + recomputed **Next due**) when a routine is confirmed. Read that page before writing: a message often restates something the vault already holds — a date told weeks ago, a status already on record, an event on a calendar page — and the dashboard alone does not show it. When the page already has the fact, leave the page alone and acknowledge it as already known ("yes, I had that noted") — never announce as newly noted what was already on record — but still run step 4: the dashboard may be the copy that is missing. New project/area → create the page (with `**Status:**`) and add its `index.md` line. Just this one owning page — sibling pages are not updated unless the fact is literally wrong on them.
...
6. **Reply in outcomes, not mechanics.** Acknowledge what was recorded or closed — never list the files it went to.
...
3. Rebuild `wiki/now.md` in full — its **Tasks** section by sweeping every wiki page's `## Tasks`, its **Someday** section from the `(someday)` tasks among them, and its **Upcoming** section by sweeping every live page's dated event bullets for the next 14 days, so a task or event ingest failed to mirror still reaches the dashboard within a day.
...
5. Run `check_vault` and fix every finding, except those under **Overdue tasks** and **Maintenance**, which are for step 6. Apply all the fixes in one turn — the edits are independent, so issue them as parallel `edit_file` calls — and run the check once more to confirm. Never fix one finding and re-check: each fix-then-check round costs two of the run's twenty turns, and a heavy day's rebuild once produced eight findings and ran out of turns before step 6 (2026-09-22). It re-verifies the rebuild mechanically — the task and event mirrors by enumeration, weekday labels against the calendar, dead links, empty sections, placeholder Status paragraphs — so a page the sweep in step 3 missed surfaces tonight instead of at the next lint. A clean report is the expected outcome, not a wasted call.
...
- Then review every live project and area page. When `fan_out` is available, use it — one worker per page, each asked to compare the page's **Status** paragraph against its own body and against the last 14 days of journal, to name open tasks the journal shows as done or moot, and to report proposed edits; apply the edits yourself. Without `fan_out`, read the pages in turn. Look for:
  ...
  - Tasks open 21+ days → surface them — except `(someday)` ones, which are open by design.
  - Read the **Someday** list out in the weekly message: how many, their titles, and whether any should get a date now. A task the user schedules gets its `(due …)` and loses the marker; one they drop is closed. Skip this when the list is empty.
