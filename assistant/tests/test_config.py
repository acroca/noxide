...
def test_load_config_with_empty_toml_does_not_raise(tmp_path: Path) -> None:
    ...
    assert "telegram_bot_token" not in Config.model_fields
...
def test_telegram_keys_are_ignored(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """A config from before Telegram support was removed still loads; its
    section and env vars are simply ignored."""
    ...
    assert cfg.pwa_port == 9000
    assert not {"telegram_bot_token", "allowed_user_ids", "default_chat_id", "pwa_enabled"} & set(Config.model_fields)
